...
public class Trainee {
    ...
    public static Double getResult(List<List<Double>> parameters, List<Double> variables) {
        double result = 0.0;

        for (List<Double> doubles : parameters) {
            double minRes = 1.0;

            for (int i = 0; i < doubles.size() - 1; i++) {
                if (minRes != 0.0) {
                    for (int j = 0; j < variables.size(); j++) {
                        if (doubles.get(i) == j + 1) {
                            minRes *= variables.get(j);
                        }
                    }
                }
            }

            minRes *= doubles.get(doubles.size() - 1);

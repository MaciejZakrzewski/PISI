...
public class Scaler {
    ...
    private static void saveMinAndMax(List<String> fileNames) throws IOException {
        ...
        saveMinAndMaxToFile(transposedMatrix);
    }
    ...
    private static void saveMinAndMaxToFile(List<List<Double>> transposedMatrix) {
        for (List<Double> column : transposedMatrix) {
            System.out.println(Collections.min(column) + DELIMITER + Collections.max(column));
        }
    }

    private static Double scaleTarget(Double target, Double colMin, Double colMax) {
        return ((target - colMin) / (colMax - colMin)) * 2.0 - 1.0;
    }

    private static Double unscaleTarget(Double target, Double colMin, Double colMax) {
